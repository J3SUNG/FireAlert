...
// 지도 그리기 함수
export function drawSimpleMap(containerId: string, items: FireItem[]) {
  ...
  items.forEach(item => {
    ...
    const coords = getApproximateCoords(mainLocation, subLocation);
    ...
    // 산불 정보 박스 스타일 설정
    const isCompleted = isCompletedStatus(item.status || '', parseInt(String(item.percentage)) || 0);
