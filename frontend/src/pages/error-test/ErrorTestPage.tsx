...
const ErrorTestPage: React.FC = () => {
  ...
  const triggerGeneralError = () => {
    const error = errorService.createError("일반적인 에러 메시지", ErrorType.UNKNOWN, {
  ...
  const triggerNetworkError = () => {
    const error = errorService.createError("네트워크 연결이 끊겼습니다", ErrorType.NETWORK, {
  ...
  const triggerAuthError = () => {
    const error = errorService.createError("인증이 만료되었습니다", ErrorType.AUTH, {
      severity: ErrorSeverity.ERROR,
    });
    errorService.handleError(error);
  ...
  const triggerValidationError = () => {
    const error = errorService.createError("입력값이 올바르지 않습니다", ErrorType.VALIDATION, {
      severity: ErrorSeverity.WARNING,
    });
    errorService.handleError(error);
  ...
  const triggerCriticalError = () => {
    const error = errorService.createError("심각한 시스템 오류가 발생했습니다", ErrorType.UNKNOWN, {
